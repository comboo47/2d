...
static func substract(_value:float)->AttributeModifier:
...
static func force_Set_Value(_value:float)->AttributeModifier:
...
func operate(_base_value:float)->float:
	match  type:
		...
		OperationType.DIVIDE:return 0 if is_zero_approx(value) else _base_value / value
